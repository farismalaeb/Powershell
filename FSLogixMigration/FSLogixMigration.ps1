...
#Parameters:
PickUpFolder: The location where the script will read the Move request. excpected value any share folder \\Sharefolder\... 
VHDorVHDX: the format of the Old FSLogix container, possible value "VHD" or "VHDX".
OldProfilePath: the location of the old FSLogix containers. Expected value: \\FileServer\FSLogix Profile 
NewDeliveryCTRL: The New XenDesktop environement delivery controller name. Expected value MyVDIEnv.domain.local
...
[Cmdletbinding()]
param(
[parameter(Mandatory=$true)]$PickUpFolder,
[parameter(Mandatory=$false)]
        [ValidateSet("VHD", "VHDX")]$VHDorVHDX='VHDX',
[parameter(Mandatory=$true)]$OldProfilePath,
[parameter(Mandatory=$true)]$NewDeliveryCTRL
)
...
$ReadAllFiles=Get-ChildItem -Filter *.dat -Path $PickUpFolder    
    ...
    foreach($File in $ReadAllFiles){      
        $UserToMigrate=Get-Content $File.FullName
        ...
        $UserDisk=Get-ChildItem -Filter "*.$($VHDorVHDX)" -Path $OldProfilePath -Recurse | Where-Object {((($_.Directory.name.Split("_"))[0]) -like $UserSid) -or ((($_.Directory.name.Split("_"))[1]) -like $UserSid)}
        try{
