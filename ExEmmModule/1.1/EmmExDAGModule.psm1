...
Function Start-EMMDAGEnabled {
    ...
    Param(
        ...
        Process{
            ...
            $false{$checkQReady=QueueFailure
            ...
            AddEmptylines -numberoflines 2 -MessageToIncludeAtTheEnd "Cluster management, if requested." -MessageColor Yellow
                    switch($IgnoreCluster){
                    $true {$step3=Set-EMMClusterConfig -ClusterNode $ServerForMaintenance -PauseOrResume PauseThisNode -IgnoreCluster}
                    $false {$step3=Set-EMMClusterConfig -ClusterNode $ServerForMaintenance -PauseOrResume PauseThisNode}
                    }
...
Function Stop-EMMDAGEnabled {
    ...
    Param(
        ...
        Process{
            ...
            AddEmptylines -numberoflines 2 -MessageToIncludeAtTheEnd "Configuring cluster if required..." -MessageColor Yellow
            switch($IgnoreCluster){
            $true {$outstep1=Set-EMMClusterConfig -ClusterNode $ServerInMaintenance -PauseOrResume ResumeThisNode -IgnoreCluster}
...
[parameter(mandatory=$False)][switch]$IgnoreCluster
    ...
    Process{
        if ($IgnoreCluster){return "Cluster Management was ignored"}
...
Write-Host "Welcome to EMM (Exchange Maintenance Module) 1.7" -ForegroundColor Green
